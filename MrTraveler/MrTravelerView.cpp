...
void CMrTravelerView::OnDraw(CDC* pDC)
{
	...
	int firstDay = curDay - (curDate % 7 - 1);//현재 월 1일 요일
	...
	CFont font;
	font.CreatePointFont(400, _T("굴림체")); 
	CString strMonth;
	...
	for (int day = 0; day < 7; day++) {
		...
		CFont font;
		font.CreatePointFont(150, _T("굴림체"));
		pDC->SetBkColor(RGB(154, 202, 235));
		pDC->SetTextColor(RGB(255, 255, 255));
		pDC->SelectObject(&font);
		if (day == 0)
	...
	for (int day = firstDay-1; day < end_of_mon[curMonth-1]+firstDay-1; day++) {
		...
		CFont font;
		font.CreatePointFont(100, _T("새돋움"));
		CString strDate;
	...
	for (int i = 0; i < 6; i++) {
		...
		CFont font;
		font.CreatePointFont(100, _T("굴림체"));
		pDC->SetBkColor(RGB(154, 202, 235));
		pDC->SetTextColor(RGB(255, 255, 255));
		pDC->SelectObject(&font);
		if (i == 0)
			...
			pDC->TextOut(25 + i * 200 + 50, 860, _T("할 일"));
		else if (i == 2)
